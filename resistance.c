...
pthread_mutex_t wait_for_resource_mutex;
...
void* resource_assign_listener(void* t){
	...
	while(1){
		...
		if(first_in_queue == my_node_state->node_data->id) {
			...
			printf("[RESOURE_ASSIGN] Process[%d] received his resource assignment\n", my_node_state->node_data->id);
		}
		else {
...
void* resource_request_listener(void* t){ // nasluchiwanie na prosby o zasob
	...
	while(1){
		...
		if(my_node_state->node_data->id == my_node_state->resource_owner) {
			if(my_node_state->using_resource == 0 && fifo_is_empty(my_node_state->resource_request_fifo)){
...
void request_for_resource() {
	...
	if(my_node_state->resource_owner == my_node_state->node_data->id){
   	...
   	pthread_mutex_lock(&wait_for_resource_mutex);
   	while(my_node_state->resource_owner != my_node_state->node_data->id){
   		pthread_mutex_unlock(&sent_resource_mutex);
   		pthread_cond_wait(&resource_cv, &wait_for_resource_mutex);
   	}
	pthread_mutex_unlock(&wait_for_resource_mutex);
...
void organize_meeting(){
	...
	//print_fifo(my_node_state->resource_request_fifo);
	first_in_queue = pop(my_node_state->resource_request_fifo);
	if(first_in_queue != -1){
...
int main(int argc, char** argv) {
	...
	pthread_mutex_init(&wait_for_resource_mutex, NULL);
